...
u32 m_latitude;				//纬度 分扩大100000倍,实际要除以100000
u32 m_longitude;			    //经度 分扩大100000倍,实际要除以100000	
u8 m_svnum;					//可见卫星数
u8 m_nshemi;					//北纬/南纬,N:北纬;S:南纬				  
u8 m_ewhemi;					//东经/西经,E:东经;W:西经
u8 m_gpssta;					//GPS状态:0,未定位;1,非差分定位;2,差分定位;6,正在估算.				  
u8 m_posslnum;				//用于定位的卫星数,0~12.
u8 m_fixmode;					//定位类型:1,没有定位;2,2D定位;3,3D定位	
u16 m_pdop;					//位置精度因子 0~500,对应实际值0~50.0
u16 m_hdop;					//水平精度因子 0~500,对应实际值0~50.0
u16 m_vdop;					//垂直精度因子 0~500,对应实际值0~50.0 
int m_altitude;			 	//海拔高度,放大了10倍,实际除以10.单位:0.1m	 
int m_speed;					//地面速率,放大了1000倍,实际除以10.单位:0.001公里/小时	
...
//	len=p1[7]-'0';								//得到GPGSV的条数
	...
	if(posx!=0XFF) m_svnum=NMEA_Str2num(p1+posx,&dx);
	/*for(i=0;i<len;i++)
	{	 
		p1=(u8*)strstr((const char *)p,"$GPGSV");  
		for(j=0;j<4;j++)
		{	  
			...
			if(posx!=0XFF)m_slmsg[slx].num=NMEA_Str2num(p1+posx,&dx);	//得到卫星编号
			else break; 
			posx=NMEA_Comma_Pos(p1,5+j*4);
			if(posx!=0XFF)m_slmsg[slx].eledeg=NMEA_Str2num(p1+posx,&dx);//得到卫星仰角 
			else break;
			posx=NMEA_Comma_Pos(p1,6+j*4);
			if(posx!=0XFF)m_slmsg[slx].azideg=NMEA_Str2num(p1+posx,&dx);//得到卫星方位角
			else break; 
			posx=NMEA_Comma_Pos(p1,7+j*4);
			if(posx!=0XFF)m_slmsg[slx].sn=NMEA_Str2num(p1+posx,&dx);	//得到卫星信噪比
			else break;
			slx++;	   
		}   
...
void NMEA_GPGGA_Analysis(u8 *buf)
{
	...
	if(posx!=0XFF)m_gpssta=NMEA_Str2num(p1+posx,&dx);	
	posx=NMEA_Comma_Pos(p1,7);								//得到用于定位的卫星数
	if(posx!=0XFF)m_posslnum=NMEA_Str2num(p1+posx,&dx); 
	posx=NMEA_Comma_Pos(p1,9);								//得到海拔高度
	if(posx!=0XFF)
		m_altitude=NMEA_Str2num(p1+posx,&dx);  
}
//分析GPGSA信息
//gpsx:nmea信息结构体
//buf:接收到的GPS数据缓冲区首地址
...
//	u8 i;   
	...
	if(posx!=0XFF)m_fixmode=NMEA_Str2num(p1+posx,&dx);	
	/*for(i=0;i<12;i++)										//得到定位卫星编号
	{
		posx=NMEA_Comma_Pos(p1,3+i);					 
		if(posx!=0XFF)m_possl[i]=NMEA_Str2num(p1+posx,&dx);
		else break; 
	}	*/
	posx=NMEA_Comma_Pos(p1,15);								//得到PDOP位置精度因子
	if(posx!=0XFF)m_pdop=NMEA_Str2num(p1+posx,&dx);  
	posx=NMEA_Comma_Pos(p1,16);								//得到HDOP位置精度因子
	if(posx!=0XFF)m_hdop=NMEA_Str2num(p1+posx,&dx);  
	posx=NMEA_Comma_Pos(p1,17);								//得到VDOP位置精度因子
	if(posx!=0XFF)m_vdop=NMEA_Str2num(p1+posx,&dx);  
}
//分析GPRMC信息
//gpsx:nmea信息结构体
//buf:接收到的GPS数据缓冲区首地址
void NMEA_GPRMC_Analysis(u8 *buf)
{
	...
	if(posx!=0XFF)
	{
		...
		m_utc_hour=temp/10000;
		m_utc_min=(temp/100)%100;
		m_utc_sec=temp%100;	 	 
	}	
	...
	if(posx!=0XFF)
	{
		...
		m_latitude=temp/NMEA_Pow(10,dx+2);	//得到°
		rs=temp%NMEA_Pow(10,dx+2);				//得到'		 
		m_latitude=m_latitude*NMEA_Pow(10,5)+(rs*NMEA_Pow(10,5-dx))/60;//转换为° 
	}
	posx=NMEA_Comma_Pos(p1,4);								//南纬还是北纬 
	if(posx!=0XFF)m_nshemi=*(p1+posx);					 
 	posx=NMEA_Comma_Pos(p1,5);								//得到经度
	if(posx!=0XFF)
	{												  
		temp=NMEA_Str2num(p1+posx,&dx);		 	 
		m_longitude=temp/NMEA_Pow(10,dx+2);	//得到°
		rs=temp%NMEA_Pow(10,dx+2);				//得到'		 
		m_longitude=m_longitude*NMEA_Pow(10,5)+(rs*NMEA_Pow(10,5-dx))/60;//转换为° 
	}
	posx=NMEA_Comma_Pos(p1,6);								//东经还是西经
	if(posx!=0XFF)m_ewhemi=*(p1+posx);		 
	posx=NMEA_Comma_Pos(p1,9);								//得到UTC日期
	if(posx!=0XFF)
	{
		temp=NMEA_Str2num(p1+posx,&dx);		 				//得到UTC日期
		m_utc_date=temp/10000;
		m_utc_month=(temp/100)%100;
		m_utc_year=2000+temp%100;	 	 
	} 
}
...
void NMEA_GPVTG_Analysis(u8 *buf)
{
	...
	if(posx!=0XFF)
	{
		m_speed=NMEA_Str2num(p1+posx,&dx);
		if(dx<3)m_speed*=NMEA_Pow(10,3-dx);	 	 		//确保扩大1000倍
	}
}  
//提取NMEA-0183信息
//gpsx:nmea信息结构体
...
{
	NMEA_GPGSV_Analysis(buf);	//GPGSV解析
	NMEA_GPGGA_Analysis(buf);	//GPGGA解析 	
	NMEA_GPGSA_Analysis(buf);	//GPGSA解析
	NMEA_GPRMC_Analysis(buf);	//GPRMC解析
	NMEA_GPVTG_Analysis(buf);	//GPVTG解析
}
